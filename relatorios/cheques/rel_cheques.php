...
<div id="rel_clientes">
  <p>RELATÓRIO DE CHEQUES</p><hr>
  <form id="form1" name="form1" method="post" action="../relatorios/cheques/cheques_paisagem.php?t=apagar">
<label for="data_incial"></label>
      &nbsp;&nbsp;&nbsp;<br />
      <table width="100%">
        <tr>
          <td>PERÍODO :</td>
          <td><input type="text" name="data_inicial"  id="data_1" />
a
  <label for="data_final"></label>
  <input type="text" name="data_final"  id="data_2" /></td>
        </tr>
        <tr>
          <td>FAVORECIDO:
          <label for="id_favorecido2"></label></td>
          <td><select name="id_favorecido" id="id_favorecido">
            <option value="">Escolha o Favorecido</option>
            <?php
do {  
?>
            <option value="<?php echo $row_seleciona_favorecidos['id']?>"><?php echo $row_seleciona_favorecidos['nome']?></option>
            <?php
} while ($row_seleciona_favorecidos = mysql_fetch_assoc($seleciona_favorecidos));
  $rows = mysql_num_rows($seleciona_favorecidos);
  if($rows > 0) {
      mysql_data_seek($seleciona_favorecidos, 0);
	  $row_seleciona_favorecidos = mysql_fetch_assoc($seleciona_favorecidos);
  }
?>
          </select></td>
...
do {  
?>
            <option value="<?php echo $row_seleciona_bancos['id']?>"><?php echo $row_seleciona_bancos['nome']?></option>
            <?php
} while ($row_seleciona_bancos = mysql_fetch_assoc($seleciona_bancos));
  $rows = mysql_num_rows($seleciona_bancos);
  if($rows > 0) {
      mysql_data_seek($seleciona_bancos, 0);
	  $row_seleciona_bancos = mysql_fetch_assoc($seleciona_bancos);
  }
?>
          </select></td>
        ...
        <tr>
          <td>CUSTODIA:
  &nbsp;</td>
          <td><select name="id_custodia" id="id_custodia">
            <option value="" selected="selected">Selecione Status</option>
            <?php
do {  
?>
            <option value="<?php echo $row_seleciona_custodias['id']?>"><?php echo $row_seleciona_custodias['nome']?></option>
            <?php
} while ($row_seleciona_custodias = mysql_fetch_assoc($seleciona_custodias));
  $rows = mysql_num_rows($seleciona_custodias);
  if($rows > 0) {
      mysql_data_seek($seleciona_custodias, 0);
	  $row_seleciona_custodias = mysql_fetch_assoc($seleciona_custodias);
  }
?>
          </select></td>
        ...
        <tr>
          <td>SACADO:</td>
          <td><select name="id_sacado" id="id_sacado">
            <option value="" selected="selected">Selecione Status</option>
            <?php
do {  
?>
            <option value="<?php echo $row_seleciona_sacados['id']?>"><?php echo $row_seleciona_sacados['nome']?></option>
            <?php
} while ($row_seleciona_sacados = mysql_fetch_assoc($seleciona_sacados));
  $rows = mysql_num_rows($seleciona_sacados);
  if($rows > 0) {
      mysql_data_seek($seleciona_sacados, 0);
	  $row_seleciona_sacados = mysql_fetch_assoc($seleciona_sacados);
  }
?>
          </select></td>
