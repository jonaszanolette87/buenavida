...
function Header()
{
	...
	//Move to the right
	$this->Cell(40);
	$this->Cell(90,10,'Email:  contato@brasilcastanha.com.br',0,0,'L');
	$this->Ln(5);
...
if (isset($_POST['id_cliente']) && $_POST['id_cliente'] !='' && isset($_POST['data_inicial']) && $_POST['data_inicial'] !='' && isset($_POST['data_final']) && $_POST['data_final'] !='' ){
	...
	$busca = mysql_query("SELECT v.*, c.nome as cliente, sum(vf.valor) as valor 
...
{
	$busca = mysql_query("SELECT v.*, c.nome as cliente, sum(vi.valortotal) as valor FROM vendas v
...
$pdf->SetX(120);
$pdf->Cell(40, 5, "R$ ".$resultado['valor']);
