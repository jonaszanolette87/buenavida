...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE compras SET id_fornecedor=%s, datacompra=%s WHERE id=%s",
  ...
  $Result1 = mysql_query($updateSQL, $conn) or die(mysql_error());
	...
	$sql = "UPDATE contasapagar SET id_fornecedor = $id_fornecedor";
  ...
  $updateGoTo = "../inicio/principal.php?pg=compras";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_seleciona_compras = "-1";
if (isset($_GET['id'])) {
  $colname_seleciona_compras = $_GET['id'];
}
mysql_select_db($database_conn, $conn);
$query_seleciona_compras = sprintf("SELECT * FROM compras WHERE id = %s", GetSQLValueString($colname_seleciona_compras, "int"));
$seleciona_compras = mysql_query($query_seleciona_compras, $conn) or die(mysql_error());
$row_seleciona_compras = mysql_fetch_assoc($seleciona_compras);
$totalRows_seleciona_compras = mysql_num_rows($seleciona_compras);
...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="2">
    <tr>
      ...
      <th width="908" class="Titulo16">ALTERA COMPRA:</th>
  ...
  <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
    <table width="950" align="center">
      <tr valign="baseline">
        ...
        <td width="96%"><?php echo $row_seleciona_compras['id']; ?>  | FORNECEDOR: 
           <select name="id_fornecedor" id="id_fornecedor">
           <option value="">Escolha  o Fornecedor</option>
             <?php
do {  
?>
             <option value="<?php echo $row_seleciona_fornecedores['id']?>"<?php if (!(strcmp($row_seleciona_fornecedores['id'], $row_seleciona_compras['id_fornecedor']))) {echo "selected=\"selected\"";} ?>><?php echo $row_seleciona_fornecedores['nome']?></option>
             <?php
...
?>
           ...
           DATA DA COMPRA
           <input name="datacompra" type="text" id="data_1" value="<?php if($row_seleciona_compras['datacompra'] == '0000-00-00' || $row_seleciona_compras['datacompra'] == 'NULL'){ 
		    $data=date("d/m/Y");
			echo $data;
		  
		   } 
		   else {
		   echo databrasil($row_seleciona_compras['datacompra']);
      ...
      <tr valign="baseline">
        <td colspan="2" align="left" nowrap="nowrap"><iframe width="950px" height="200px" frameborder="0" src="../compras_itens/index.php?id_compra=<?php echo $row_seleciona_compras['id']?>"></iframe></td>
      </tr>
      <tr valign="baseline">
        <th colspan="2" align="left" nowrap="nowrap">FORMA DE PAGAMENTO</th>
      </tr>
      <tr valign="baseline">
        <td colspan="2" align="left" nowrap="nowrap"><iframe width="950px" height="150px" frameborder="0" src="../compras_fechamento/index.php?id_compra=<?php echo $row_seleciona_compras['id']?>"></iframe></td>
      </tr>
      <tr valign="baseline">
    ...
    <input type="hidden" name="id" value="<?php echo $row_seleciona_compras['id']; ?>" />
  </form>
  <p>&nbsp;</p>
<?php
