...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1_altreservas")) {
	...
	$sql_busca_ocupado = "SELECT * FROM reservas_dias WHERE dia='".$_POST['chegada']."'  AND id_quarto=".$_POST['id_quarto']."";
	
	$result = mysql_query($sql_busca_ocupado,$conn);
	...
	{
	echo "<script> Alert('ja existe reserva para esse dia!');</script>";
	echo "Ja existe reserva para esse dia ";
	}
  ...
  valor_reserva=%s,formadepagamento=%s,cafedamanha=%s, comissao=%s, valor_retido=%s ,
   cobro_adiantamento=%s,formadepagamento_checkin =%s,status=%s, observacoes=%s  WHERE id=%s",
...
//$dia = date('Y-m-d', strtotime($dia_chegada. "+".$i." days"));
   ...
   $insertSQL2 = sprintf("UPDATE reservas_dias SET dia=%s,status=%s,id_quarto=%s    WHERE id_reserva=%s",
...
mysql_select_db($database_conn, $conn);
	$sql_busca_ocupado = "SELECT * FROM reservas_dias WHERE dia='".$dia."'  AND id_quarto=".$_POST['id_quarto']."";
	
	$result = mysql_query($sql_busca_ocupado,$conn);
	
	$linha = mysql_fetch_assoc($result);
	$linhas = mysql_num_rows($result);
	$linhas_qtd = $linhas_qtd + $linhas;
   ...
   $updateSQL2 = sprintf("UPDATE reservas_dias SET  id_quarto=%s,status=%s, WHERE id_reserva=%s",
...
<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="2">
    <tr>
      <th width="45"><img src="../imagens/campainha.png" alt="" width="30" height="30" align="absmiddle" /></th>
      <th width="662" class="Titulo16">ALTERA RESERVAS:</th>
      <th width="48" class="Titulo16"><a href="../voucher/index.php?ID=<?php echo $row_seleciona_reservas['id']; ?>" target="_blank"><img src="../imagens/configuracoes.gif" width="28" height="28" /></a></th>
      <th width="59" class="Titulo16"><a href="#" onclick="closeMessage()"><img src="../imagens/botao_fechar.png" width="25" height="25" alt="fechar" /></a></th>
    </tr>
</table>
