...
$maxRows_seleciona_compras_itens = 10;
...
mysql_select_db($database_conn, $conn);
$query_seleciona_compras_itens = sprintf("SELECT vi.*, mp.nome as produto FROM compras_itens vi
...
do {  
?>
          <option value="<?php echo $row_seleciona_produtos['id']?>"><?php echo $row_seleciona_produtos['nome']?><?php echo "<b> R$ ".$row_seleciona_produtos['precocusto']."</b>"?></option>
          <?php
...
?>
      ...
      <td width="73%" align="left" nowrap="nowrap">QTD:
        <input type="text" id="qtd" name="qtd" value="1" size="3" onblur="soma()" />
        VALOR:
      <input type="text" id="precocusto" name="precocusto" value="" size="5" onblur="soma()" />
      VL. TOTAL:
      <input type="text" id="valortotal" name="valortotal" value="" size="4"  onblur="soma()" />
