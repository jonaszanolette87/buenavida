...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1_altreservas")) {
  ...
  valor_reserva=%s,formadepagamento=%s,cafedamanha=%s, comissao=%s, valor_retido=%s ,
   cobro_adiantamento=%s,formadepagamento_checkin =%s,status=%s,checkin=%s, observacoes=%s  WHERE id=%s",
...
$dia = date('Y-m-d', strtotime($dia_chegada. "+".$i." days"));
  ...
  $updateGoTo = "../inicio/principal.php?t=checkin";
  if (isset($_SERVER['QUERY_STRING'])) {
...
?>
          ...
          <tr valign="baseline">
            ...
            <td><input name="id" type="hidden" id="id" value="<?php echo $row_seleciona_reservas['id']?>" />
            <input name="id_permissao" type="hidden" id="id_permissao" value="<?php echo $row_permissoes['id']?>" />
            <input name="checkin" type="hidden" id="checkin" value="<?php  $date = date('Y-m-d H:i:s');  echo $date;
			?>" /></td>
          </tr>
          <tr valign="baseline">
            <td nowrap="nowrap" align="right">&nbsp;</td>
