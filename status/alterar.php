...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE status SET numero=%s,nome=%s,valor=%s WHERE id=%s",
  ...
  $updateGoTo = "../inicio/principal.php?t=status";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_seleciona_status = "-1";
if (isset($_GET['id'])) {
  $colname_seleciona_status = $_GET['id'];
}

$id = $_GET['id'];

mysql_select_db($database_conn, $conn);
$query_seleciona_status = sprintf("SELECT * FROM status WHERE id = %s", GetSQLValueString($colname_seleciona_status, "int"));
...
$totalRows_seleciona_status = mysql_num_rows($seleciona_status);
      ...
      <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
        <table width="100%" align="center">
          ...
          <tr valign="baseline">
            ...
            <td><input name="nome" type="text" id="nome" value="<?php echo htmlentities($row_seleciona_status['nome'], ENT_COMPAT, 'utf-8'); ?>" size="44" /></td>
          </tr>
          <tr valign="baseline">
            <td nowrap="nowrap" align="right"><input name="id2" type="hidden" id="id" value="<?php echo $row_seleciona_status['id']?>" /></td>
            <td><input type="submit" value="Confirmar" />
            <input type="button" class="btn1" onclick="closeMessage()" value="Cancelar" /></td>
          </tr>
        </table>
        <input type="hidden" name="MM_update" value="form1" />
        <input type="hidden" name="id" value="<?php echo $row_seleciona_status['id']; ?>" />
      </form>
    <p>&nbsp;</p></td>
  </tr>
</table>
